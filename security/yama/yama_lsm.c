...
int ptrace_scope = YAMA_SCOPE_RELATIONAL;
...
void yama_task_free(struct task_struct *task)
{
	...
	yama_free_task_filter(task);
}

int yama_task_copy(struct task_struct *task)
{
	return yama_copy_task_filter(task);
}
			   ...
			   unsigned long arg4, unsigned long arg5)
{
	...
	case PR_YAMA_OPTS:
		get_task_struct(myself);
		rc = yama_prctl_opts(myself, arg2, arg3, arg4);
		put_task_struct(myself);
		break;
	}
...
static struct security_hook_list yama_hooks[] = {
	...
	LSM_HOOK_INIT(task_copy, yama_task_copy),
...
void __init yama_add_hooks(void)
{
	...
	if (yama_tasks_init())
		panic("Yama: tasks initialization failed.\n");
}
